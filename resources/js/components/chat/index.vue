        ...
        <div class="chat_friend_interface chat_shodow" v-show="!minimizes">
            ...
            <div class="chat_friend_interface_content">
                <dl class="chat_handle" v-for="(v,k) in chat_friend.data" :key="k" @click="onChatHandle(v.store)">
                    <dt><img :src="v.store.avatar||require('@/asset/user/user_default.png')" alt=""></dt>
                    <dd>{{v.store.nickname||'加载中...'}}<span v-show="v.not_read_count>0"></span></dd>
            ...
            <div class="chat_interface" v-if="onChat.length>0">
                ...
                <div class="chat_interface_right">
                    <div class="chat_interface_right_top">{{onChat[onChatIndex].nickname||'加载中...'}}<a-icon class="chat_handle"  @click="onChat = [];onChatIndex = 0;" type="close" /></div>
                            ...
                            <div>
                                <div :class="v.send_type==0?'chat_interface_msg_item me':'chat_interface_msg_item'" v-for="(v,k) in onChat[onChatIndex].chat_msg.data" :key="k">
                                    <div class="chat_interface_msg_avatar">
                                        <img :src="v.send_type==1?v.avatar:userInfo.avatar||require('@/asset/user/user_default.png')" alt="">
                                    </div>
                                    <div class="chat_interface_msg_time"><span>{{v.send_type==1?v.nickname:userInfo.nickname||'-'}}</span> {{v.created_at}}</div>
